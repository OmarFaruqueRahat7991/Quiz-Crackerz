...
const Questions = ({eachquiz}) => {
    const {question , options} = eachquiz;
    return (
        <div className='p-10 border-2	border-gray-500	border-solid mx-10 mb-5'>
            <h1 className='text-lg font-semibold'>{question} <small> [See the Answer <FontAwesomeIcon icon={faEye}></FontAwesomeIcon> ]</small> </h1>
            <div className='grid md:grid-cols-2 m-5 p-15 gap-4'>
                {
                    options.map(option => <EachQuiz
                        key={option.id}
                        option={option}
                        ></EachQuiz>)
                }
